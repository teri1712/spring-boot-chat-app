...
public class EngagementServiceImpl implements EngagementService, EngagementApi, DirectMappingApi {
    ...
    public static final Integer DEFAULT_GROUP_CAPACITY = 1000;

    private final ParticipantRepository participants;
    private final ChatViewRepository chatViews;
    ...
    public Optional<ChatPolicyInfo> find(String chatId, UUID userId) {
        ...
        Chat chat = chatViews.findById(chatId).orElse(null);
    ...
    public DirectMapping findDirectMapping(UUID userId, UUID partnerId) {
        ...
        return chatViews.findById(chatId).map(chat -> {
            Participant participant = participants.findById(new ParticipantId(userId, chatId)).orElse(null);
            engagementPolicy.applyRead(participant, chat);
    ...
    @Override
    public void add(String chatId, Set<UUID> partners) {

        Chat chat = chats.findById(chatId).orElseThrow();
        Set<Participant> pSet = partners.stream()
            .map(partnerId -> new Participant(partnerId, chatId))
            .collect(Collectors.toSet());

        chatPolicyService.apply(pSet, chat);
        participants.saveAll(pSet);
        applicationEventPublisher.publishEvent(new ParticipantAdded(chatId, partners, Instant.now()));
        chats.save(chat);
    }
    ...
    public ChatPolicyInfo createGroup(UUID callerId, Set<UUID> participants) {
        ...
        Chat chat = groupChatFactory.create(creators, DEFAULT_GROUP_CAPACITY);
