...
public class PresenceControllerTest extends BaseTestClass {
      ...
      void givenAliceAndBobAreOnline_whenAliceListsOnline_thenReturnsBobOnly() throws Exception {
            ...
            presenceSetter.set(alice.getId(), alice.getUsername(), alice.getClaims().name(), alice.getClaims().avatar(), Instant.now());
            presenceSetter.set(bob.getId(), bob.getUsername(), bob.getClaims().name(), bob.getClaims().avatar(), Instant.now());
