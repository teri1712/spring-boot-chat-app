...
fun main(args: Array<String>) {
    ...
    transactionOperations.executeWithoutResult {
        ...
        eventStore.save(event = TextEvent(Chat(luffy, nami), luffy, "Hello").apply {
            createdTime = System.currentTimeMillis() - 5 * 60 * 1000
        })
        eventStore.save(event = TextEvent(Chat(luffy, chopper), luffy, "Ekk").apply {
            createdTime = System.currentTimeMillis() - 10 * 1000
        })
        eventStore.save(event = TextEvent(Chat(luffy, chopper), luffy, "Vcl").apply {
